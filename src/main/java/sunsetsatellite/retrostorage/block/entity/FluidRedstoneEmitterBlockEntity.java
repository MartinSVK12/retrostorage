...
import sunsetsatellite.retrostorage.block.FluidRedstoneEmitterBlock;
...
public class FluidRedstoneEmitterBlockEntity extends NetworkDeviceBlockEntity implements ScreenActionListener {
    ...
    public Filter filter = new Filter(0, 1);
    ...
    public void tick() {
        ...
        world.setBlockState(x, y, z, world.getBlockState(x, y, z).with(FluidRedstoneEmitterBlock.ACTIVE, isActive));
        if (getController() != null) {
            if (filter.getFluid(0, null) != null) {
                int id = filter.getFluid(0, null).fluid.getFlowingBlock().id;
                //int dmg = filter.getFluid(0,null).getDamage();
                //NbtCompound tag = filter.getStack(0).getStationNbt();
                long count = getController().countFluids(id);
                //if (useMeta) {
    ...
    public String getName() {
        return "container.retrostorage.fluidRedstoneEmitter";
    }
