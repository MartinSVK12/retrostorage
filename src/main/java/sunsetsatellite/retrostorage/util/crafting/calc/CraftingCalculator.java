...
public class CraftingCalculator {
    ...
    public CalculationResult calculate() {
        ...
        if (requested != null) {
            ...
            if (recipe.getType() == sunsetsatellite.retrostorage.util.crafting.CraftableType.PROCESS) {
    ...
    private void calculateInternal(int qty, ItemStackList source, FluidStackList fluidSource, ItemStackList results, FluidStackList fluidResults, NetworkCraftable recipe, boolean root) throws CraftingCalculationException {
        ...
        ArrayList<ItemStack> inputs = MachineEssentials.condenseItemList(RetroStorage.getRecipeItems(recipe));
        ArrayList<FluidStack> fluidInputs = MachineEssentials.condenseFluidList(RetroStorage.getRecipeFluids(recipe));
    ...
    private void calculateFluids(int qty, ItemStackList source, FluidStackList fluidSource, ItemStackList results, FluidStackList fluidResults, ArrayList<FluidStack> inputs, Node node) throws CraftingCalculationException  {
        ...
        for (FluidStack input : inputs) {
            ...
            FluidStack fromSelf = fluidResults.getById(input.fluid.blockId());
            FluidStack fromNetwork = fluidSource.getById(input.fluid.blockId());
            ...
            while (remaining > 0) {
                if (fromSelf != null) {
                    ...
                    fluidResults.removeById(fromSelf.fluid.blockId(), toTake, false);

                    remaining -= toTake;

                    fromSelf = fluidResults.getById(input.fluid.blockId());
                }
                ...
                if (fromNetwork != null && remaining > 0) {
                    ...
                    fluidSource.removeById(fromNetwork.fluid.blockId(), toTake, false);

                    remaining -= toTake;

                    fromNetwork = fluidSource.getById(input.fluid.blockId());
                ...
                if (remaining > 0) {
                    ...
                    if (subRecipe != null) {
                        ...
                        if (subRecipe.getType() == sunsetsatellite.retrostorage.util.crafting.CraftableType.PROCESS) {
