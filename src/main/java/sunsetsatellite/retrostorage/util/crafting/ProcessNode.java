...
public class ProcessNode extends Node {
    ...
    public void update(NetworkController network, NodeList nodes, ItemStackList internalStorage, FluidStackList internalFluidStorage, CraftingTask craftingTask) {
        Processor processor = network.findProcessor(getPattern());
    ...
    public int getNeeded(ItemStack stack) {
        return (int) (singleItemsToReceive.count(stack.itemId, stack.getDamage(), stack.getStationNbt()) * totalQuantity - itemsReceived.count(stack.itemId, stack.getDamage(), stack.getStationNbt()));
    }
    ...
    public void updateFinishedQuantity() {
        ...
        for (ItemStack stack : singleItemsToReceive) {
            if (itemsReceived.get(stack.itemId, stack.getDamage(), stack.getStationNbt()) != null) {
                int ratioReceived = (int) (itemsReceived.count(stack.itemId, stack.getDamage(), stack.getStationNbt()) / stack.count);
