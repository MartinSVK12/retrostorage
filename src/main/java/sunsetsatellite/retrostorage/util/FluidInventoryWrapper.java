...
public class FluidInventoryWrapper implements IFluidStackList{
    ...
    public FluidInventory connected;

    public FluidInventoryWrapper(FluidInventory inventory) {
        connected = inventory;
    }
    ...
    public FluidStack add(FluidStack stack) {
        ...
        for (int i = 0; i < connected.size(); i++) {
            ...
            if(invStack == null) {
                ...
                n -= amount;
                connected.setFluidInSlot(i, stack.split(amount));
                if(n <= 0) break;
    ...
    public FluidStack remove(int slot, int amount, boolean strict) {
        List<FluidStack> contents = MachineEssentials.collectFluidStacks(connected);
    ...
    public @UnmodifiableView List<FluidStack> getStacks() {
        return MachineEssentials.collectAndCondenseFluidStacks(connected);
    }
