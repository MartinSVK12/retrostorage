...
public class ImporterScreenHandler extends FilterScreenHandler {

    public ImporterBlockEntity tile;

    public ImporterScreenHandler(PlayerInventory playerInv, ImporterBlockEntity tile) {
        this.tile = tile;

        for (int i = 0; i < 3; i++) {
            for (int l = 0; l < 3; l++) {
                addSlot(new GhostSlot(tile.filter, l + i * 3, 62 + l * 18, 17 + i * 18));
            }
