...
public class BlockLogicProcessProgrammer extends BlockLogicRotatable {
    ...
    public void onBlockRemoved(World world, int x, int y, int z, int data) {
        TileEntityProcessProgrammer tileEntityProcessProgrammer = (TileEntityProcessProgrammer) world.getTileEntity(x, y, z);
        label0:
        for (int l = 0; l < tileEntityProcessProgrammer.getContainerSize(); l++) {
            ItemStack itemstack = tileEntityProcessProgrammer.getItem(l);
            if (itemstack == null) {
                continue;
            }
    ...
    public boolean onBlockRightClicked(World world, int i, int j, int k, Player entityplayer, Side side, double xHit, double yHit) {
        if (world.isClientSide) {
            ...
            TileEntityProcessProgrammer tile = (TileEntityProcessProgrammer) world.getTileEntity(i, j, k);
            if (tile != null) {
                Catalyst.displayGui(entityplayer, tile, RetroStorage.key("gui/process_programmer"));
            }
            return true;
        }
    }
