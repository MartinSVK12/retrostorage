...
public class BlockLogicExporter extends BlockLogicNetworkDevice {


    public BlockLogicExporter(Block<?> block, Supplier<TileEntity> tileEntitySupplier, String guiId) {
        super(block, tileEntitySupplier, guiId);
    }

    public void onBlockRemoved(World world, int x, int y, int z, int data) {
        TileEntityExporter tileEntityExporter = (TileEntityExporter) world.getTileEntity(x, y, z);
        label0:
        for (int l = 0; l < tileEntityExporter.getContainerSize(); l++) {
            ItemStack itemstack = tileEntityExporter.getItem(l);
            if (itemstack == null) {
                continue;
            }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, int blockId) {
        ...
        TileEntityExporter tile = (TileEntityExporter) world.getTileEntity(x, y, z);
        if(tile != null) {
