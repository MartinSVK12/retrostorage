...
public class Network {
    ...
    public void reload(){
        removeAll();
        if(controller != null){
            HashMap<Direction, BlockInstance> candidates = scan(controller.worldObj, new Vec3i(controller.xCoord,controller.yCoord,controller.zCoord));
    ...
    public HashMap<Direction, BlockInstance> scan(World world, Vec3i pos){
        ...
        for (Direction direction : Direction.values()) {
            sides.put(direction,null);
        }

        for (Direction direction : Direction.values()) {
            TileEntity tile = direction.getTileEntity(world, pos);
            Vec3i V = direction.getVec();
