...
import net.sunsetsatellite.retrostorage.tiles.TileEntityDigitalController;
...
public class InventoryDigital implements IInventory {
	...
	public int getInventorySlotContainItem(int itemID, int itemDamage) {
		for(int i2 = 0; i2 < this.inventoryContents.length; ++i2) {
			if(this.inventoryContents[i2] != null && this.inventoryContents[i2].itemID == itemID && (this.inventoryContents[i2].getItemDamage() == itemDamage || itemDamage == -1)) {
				return i2;
			}
		}
		...
		for(int i2 = 0; i2 < this.inventoryContents.length; ++i2) {
			if(this.inventoryContents[i2] != null){
				if(this.inventoryContents[i2].itemID == stack.itemID){
					if((stack.stackTagCompound == null && this.inventoryContents[i2].stackTagCompound == null) || ((this.inventoryContents[i2].stackTagCompound).equals(stack.stackTagCompound))){
						if(this.inventoryContents[i2].isStackable()){
							if (this.inventoryContents[i2].stackSize < this.inventoryContents[i2].getMaxStackSize()){
								if(this.inventoryContents[i2].stackSize < this.getInventoryStackLimit()){
									if((!this.inventoryContents[i2].getHasSubtypes() || (this.inventoryContents[i2].getItemDamage() == stack.getItemDamage() || stack.getItemDamage() == -1))){
										return i2;
									}
								}
							}
					...
					/*&& this.inventoryContents[i2].itemID == stack.itemID
					&& ((this.inventoryContents[i2].stackTagCompound).equals(stack.stackTagCompound)||(stack.stackTagCompound == null && this.inventoryContents[i2].stackTagCompound == null))
					&& this.inventoryContents[i2].isStackable()
					&& this.inventoryContents[i2].stackSize < this.inventoryContents[i2].getMaxStackSize()
					&& this.inventoryContents[i2].stackSize < this.getInventoryStackLimit()
					&& (!this.inventoryContents[i2].getHasSubtypes() || this.inventoryContents[i2].getItemDamage() == stack.getItemDamage())) {*/
	...
	public int storePartialItemStack(ItemStack stack) {
		...
		if(i4 < 0) {
			...
			if(this.inventoryContents[i4] == null) {
				this.inventoryContents[i4] = new ItemStack(id, 0, stack.getItemDamage());
				this.inventoryContents[i4].stackTagCompound = stack.stackTagCompound;
			}
	...
	public boolean hasItems(ArrayList<ItemStack> stacks){
		for (ItemStack stack : stacks) {
			if (getItemCount(stack.itemID, stack.getItemDamage()) < stack.stackSize) {
				return false;
			}
		}
		return true;
	...
	public ArrayList<ItemStack> hasItemsReturnMissing(ArrayList<ItemStack> stacks){
		...
		for (ItemStack stack : stacks) {
			int count = getItemCount(stack.itemID, stack.getItemDamage());
			if(stack.getItemDamage() == -1){
				count = getItemCount(stack.itemID);
			}
			if (count < stack.stackSize) {
				boolean alreadyMissing = false;
				for(ItemStack misStack : missing){
					if(misStack.isItemEqual(stack)){
						misStack.stackSize += stack.stackSize - getItemCount(stack.itemID, stack.getItemDamage());
						alreadyMissing = true;
						break;
					}
				}
				...
				missingStack.stackSize = stack.stackSize - getItemCount(stack.itemID, stack.getItemDamage());
	...
	public boolean removeItems(ArrayList<ItemStack> stacks){
		...
		for (ItemStack stack : stacks){
			...
			int slot;
			int count;
			int fullCount = getItemCount(stack.itemID,stack.getItemDamage());
			if(fullCount >= stack.stackSize){
				do{
					fullCount = getItemCount(stack.itemID,stack.getItemDamage());
					if(fullCount < copy.stackSize){
						return false;
					}
					slot = getInventorySlotContainItem(stack.itemID,stack.getItemDamage());
