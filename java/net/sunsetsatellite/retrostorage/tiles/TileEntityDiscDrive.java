    ...
    implements IInventory
{
    ...
    public TileEntityDiscDrive()
    {
        ...
        virtualDisc.stackTagCompound = new NBTTagCompound();
    }
            ...
            if(network.drive == null){
                ...
                if(getStackInSlot(0) != null){
                    if(getStackInSlot(0).getItem() instanceof ItemStorageDisc){
                        ...
                        Object[] nbt = stack.stackTagCompound.getCompoundTag("disc").getTags().toArray();
                        for(Object stackTagCompound : nbt){
                            if(stackTagCompound instanceof NBTTagCompound){
                                //mod_RetroStorage.printCompound((NBTTagCompound) stackTagCompound);
                                ItemStack digitizedItem = ItemStack.loadItemStackFromNBT((NBTTagCompound) stackTagCompound);//new ItemStack(((NBTTagCompound)stackTagCompound));
                                //mod_RetroStorage.LOGGER.info(String.format("%d %d %d",digitizedItem.itemID,digitizedItem.stackSize,digitizedItem.getMetadata()));
    ...
    public void removeLastDisc() {
        if(discsUsed.size() > 0){
            ...
            Object[] V = virtualDisc.stackTagCompound.getCompoundTag("disc").getTags().toArray();
            int stacksToRemove = Math.min(virtualDisc.stackTagCompound.getCompoundTag("disc").getTags().size(), ((ItemStorageDisc) disc.getItem()).getMaxStackCapacity());
            ...
            disc.stackTagCompound.setCompoundTag("disc",nbt);
            disc.stackSize = 1;
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound)
    {
        ...
        {
            NBTTagCompound nbttagcompound1 = (NBTTagCompound)nbttaglist.tagAt(i);
            discsUsed.add(ItemStack.loadItemStackFromNBT(nbttagcompound1));//new ItemStack(nbttagcompound1));
        }
        virtualDriveMaxStacks = nbttagcompound.getInteger("MaxStacks");
        virtualDisc.stackTagCompound.setCompoundTag("disc", nbttagcompound.getCompoundTag("VirtualDisc"));
    }
    ...
    public void writeToNBT(NBTTagCompound nbttagcompound)
    {
        ...
        nbttagcompound.setTag("MaxStacks",new NBTTagInt("MaxStacks",virtualDriveMaxStacks));
        nbttagcompound.setCompoundTag("VirtualDisc",virtualDisc.stackTagCompound.getCompoundTag("disc"));
    }
    ...
    public ItemStack virtualDisc = (new ItemStack(mod_RetroStorage.virtualDisc,1));
