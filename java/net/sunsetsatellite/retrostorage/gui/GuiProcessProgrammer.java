...
import net.sunsetsatellite.retrostorage.containers.ContainerProcessProgrammer;
...
public class GuiProcessProgrammer extends GuiContainer
{
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int par3) {
        int l = mc.renderEngine.getTexture("/assets/retrostorage/gui/process_programmer.png");
    ...
    {
        super.initGui();
        processName = new GuiTextField(fontRenderer, Math.round(width / 2 - 31),Math.round(height/2 - 92),100,20);
    ...
    public void keyTyped(char c1, int i2) {
        if(processName.func_50025_j()) {
            Keyboard.enableRepeatEvents(true);
            if (c1 == Keyboard.KEY_ESCAPE) {
                Keyboard.enableRepeatEvents(false);
                processName.func_50033_b(false);
            } else processName.func_50037_a(c1, i2);
